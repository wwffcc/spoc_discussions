...
class fs:
    ...
    def createFile(self, parent, newfile, ftype):
        ...
        # DONE
        assert(ftype == 'f' or ftype == 'd')
        parent_inum = self.nameToInum[parent]
        parent_data = self.data[parent_inum]
        # print parent_data.dump()
        # print self.ibitmap.bmap
        if parent_data.getFreeEntries <= 0:
            return -1
        elif parent_data.dirEntryExists(newfile):
            return -1
        else:
            inum = 0
            #print self.ibitmap.bmap
            for b in self.ibitmap.bmap:
                if b == 0:
             #       print inum
                    break
                inum += 1

            self.ibitmap.markAllocated(inum)
            # if ftype == 'd':
            self.inodes[inum].setAll(
                'd', (parent_data.getEntry(inum - 1))[1] + 1, 2)
            self.dbitmap.markAllocated(inum)
            self.data[inum].setType('d')
            self.data[inum].addDirEntry('.', inum)
            self.data[inum].addDirEntry('..', inum)
        return inum
...
parser.add_option('-i', '--numInodes',   default=8,
                  help='number of inodes in file system',      action='store', type='int', dest='numInodes')
